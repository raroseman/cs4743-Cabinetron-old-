...
public class PartsInventoryModel {
	...
	public void addPart(Part p) throws Exception {
		try {
			addPart(p.getQuantity(), p.getQuantityUnitType(), p.getPartName(), p.getPartNumber(), p.getVendor());
		}
		catch (IOException e) {
	...
	public void addPart(Integer quantity, String unitOfQuantity, String partName, String partNumber) throws Exception {
		try {
			addPart(quantity, unitOfQuantity, partName, partNumber, "");
		}
		catch (IOException e) {
			throw new IOException(e.getMessage());
		}
	...
	public void addPart(Integer quantity, String unitOfQuantity, String partName, String partNumber, String vendor) throws Exception, IOException {
		if (quantity <= 0) {
			throw new IOException("A new item requires quantity greater than zero.");
		}
		try {
			Part p = new Part(quantity, unitOfQuantity, partName, partNumber, vendor);
	...
	public void editPart(Part partOld, int newQuantity, String newQuantityUnitType, String newName, String newPartNumber, String newVendor) throws Exception {
		int index = partsInventory.indexOf(partOld);
	//	if (index == -1) {
	//		throw new Exception("Error: the old part, " + partOld.getPartName() + " cannot be edited as it is not listed in inventory.");
	//	}
		...
		if (!partOld.getPartName().equals(newName) && findPartName(newName) != null) {
			...
			Part newPart = new Part(newQuantity, newQuantityUnitType, newName, newPartNumber, newVendor);
